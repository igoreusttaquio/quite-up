...
const schema = z.object({
  type: z.string().min(1), value: z.coerce.number().positive('Valor deve ser positivo'),
...
function TransactionModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['transactions'] }); queryClient.invalidateQueries({ queryKey: ['accounts'] }); onClose(); reset() },
  })
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-4">
          ...
          <div>
            ...
            <input type="number" step="0.01" placeholder="0,00" className={`w-full px-4 py-2.5 rounded-lg border text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 ${errors.value ? 'border-red-400' : 'border-gray-300'}`} {...register('value')} />
            {errors.value && <p className="text-red-500 text-xs mt-1">{errors.value.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Data</label>
            <input type="date" className="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" {...register('date')} />
          </div>
          <div>
          ...
          <div>
            ...
            <input className="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" {...register('description')} />
          </div>
...
export function TransactionsPage() {
  ...
  const queryClient = useQueryClient()
  const { data: transactions } = useQuery({ queryKey: ['transactions'], queryFn: () => transactionsApi.getAll(), select: (res) => res.data })
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setModalOpen(true)} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700">Nova transação</button>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {(transactions?.items ?? []).length === 0 ? (
                <tr><td colSpan={7} className="text-center py-8 text-gray-400">Nenhuma transação encontrada</td></tr>
              ) : transactions?.items.map((t) => (
                <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-600">{new Date(t.date).toLocaleDateString('pt-BR')}</td>
                  <td className="px-4 py-3"><span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${typeStyles[t.type].bg} ${typeStyles[t.type].text}`}>{typeStyles[t.type].label}</span></td>
                  ...
                  <td className="px-4 py-3"><button onClick={() => deleteMutation.mutate(t.id)} className="text-red-600 hover:text-red-700 text-xs font-medium">Excluir</button></td>
                </tr>
              ))}
            </tbody>
          </table>
