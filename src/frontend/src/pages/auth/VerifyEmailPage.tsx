    ...
    <div className="text-center space-y-5">
      {verified || verifyMutation.isSuccess ? (
        ...
        <>
          ...
          <p className="text-gray-500">Enviamos um link de verificação para seu email. Clique no link para ativar sua conta.</p>
          <button onClick={() => { const email = prompt('Digite seu email:'); if (email) resendMutation.mutate(email) }}
            disabled={resendMutation.isPending}
            className="w-full border border-indigo-600 text-indigo-600 py-2.5 rounded-lg font-medium hover:bg-indigo-50 disabled:opacity-50">
